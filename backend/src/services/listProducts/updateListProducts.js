...
const updateListProducts = async (req, res)=>{
  ...
  const listProductsExist = await ListProducts.findOne({where: {order_id: req.body.order_id, id_product: id_product}});
  if (listProducts && params.id !=listProductsExist.id){
  ...
  const newOrder = {
    ...
    total: parseFloat(order.total)+parseFloat(newListProducts.amount)-parseFloat(listProducts.amount),
    data: order.data,
    status: order.status
  }
  const orderUp = await order.update(newOrder);
  const listProductsUp = await listProducts.update(newListProducts);
  return res.status(200).json(listProductsUp);
}
module.exports = updateListProducts;
